...
function card(k,h){var L=window.LABELS||{},F=window.FIELDS||[];
  if(!h) return '<div class="card"><span class="code">'+esc(k)+'</span> '+esc(L.miss||"")+'</div>';
  var p='<div class="card"><span class="code">'+esc(h.name||k)+'</span> <small>'+esc(h.cat||"")+'</small>';
...
function find(raw){var C=window.CODES||{}; raw=(raw||"").trim().toLowerCase(); if(!raw)return null;
  ...
  for(var k in C){var hay=(k+" "+(C[k].alias||"")+" "+(C[k].name||"")).toLowerCase();
    if(hay.indexOf(raw)>=0||raw.indexOf(k.toLowerCase())>=0)return[k,C[k]];}
  return[raw,null];}
...
function table(){var C=window.CODES||{},H=window.THEAD||[];
  var rows=Object.keys(C).map(function(k){var h=C[k];
    return '<tr><td class="code">'+esc(h.name||k)+'</td><td>'+esc(h.cat||"")+'</td><td>'+esc(h.l1||"-")+'</td><td>'+esc(h.l2||"-")+'</td></tr>';}).join("");
